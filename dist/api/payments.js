...
export const createPaymentIntent = async (payload, token) => {
    ...
    try {
        const response = await apiClient.post(PAYMENT_ENDPOINTS.CREATE_INTENT, {
            ...payload,
...
export const confirmPayment = async (paymentIntentId, token) => {
    ...
    try {
        const response = await apiClient.post(PAYMENT_ENDPOINTS.CONFIRM, { paymentIntentId }, {
            headers: {
                Authorization: `Bearer ${token}`
            }
        });
 ...
 */
export const addPaymentMethod = async (paymentMethodId, setAsDefault = false, token) => {
    console.log('[Payments API] 💳 Adding payment method');
    console.log('   Payment Method ID:', paymentMethodId);
    console.log('   Set as default:', setAsDefault);
    try {
        const response = await apiClient.post(PAYMENT_ENDPOINTS.ADD_CARD, {
            paymentMethodId,
            setAsDefault
        }, {
            headers: {
 ...
 */
export const setDefaultPaymentMethod = async (paymentMethodId, token) => {
    console.log('[Payments API] ⭐ Setting default payment method:', paymentMethodId);
    try {
        await apiClient.post(PAYMENT_ENDPOINTS.SET_DEFAULT, { paymentMethodId }, {
            headers: {
                Authorization: `Bearer ${token}`
            }
        });
 ...
 * ✅ Add tip
 */
export const addTip = async (payload, token) => {
    console.log('[Payments API] 💵 Adding tip');
    console.log('   Amount: $', payload.amount);
    console.log('   Ride:', payload.rideId);
    try {
        const response = await apiClient.post(PAYMENT_ENDPOINTS.TIP(payload.rideId), {
            amount: Math.round(payload.amount * 100), // Convertir a centavos
...
export const createConnectAccount = async (token) => {
    ...
    try {
        const response = await apiClient.post(PAYMENT_ENDPOINTS.CREATE_CONNECT_ACCOUNT, {}, {
            headers: {
                Authorization: `Bearer ${token}`
            }
        });
 ...
 */
export const getConnectAccountStatus = async (token) => {
    console.log('[Payments API] 📊 Getting Connect account status');
    try {
        const response = await apiClient.get(PAYMENT_ENDPOINTS.CONNECT_ACCOUNT_STATUS, {
            headers: {
                Authorization: `Bearer ${token}`
            }
        });
